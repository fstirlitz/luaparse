...
(function (root, name, factory) {
  ...
  // Add identifier name to the current scope.

  function scopeIdentifier(name) {
    scopes[scopeDepth].push(name);
  }

  // Attach scope information to the identifier node.

  function attachScope(node) {
    node.isLocal = indexOf.call(scopes[scopeDepth], node.name) !== -1;
  }
  ...
  function parseLocalStatement() {
    if (Identifier === token.type) {
      ...
      do {
        ...
        if (options.scope) scopeIdentifier(token.value);
  ...
  function parseIdentifier() {
    ...
    next();
    var node = ast.identifier(identifier);
    if (options.scope) attachScope(node);
    return node;
  }
