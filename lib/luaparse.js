...
(function (root, name, factory) {
  ...
  var ast
    ...
    , errors = {
      ...
      , expectedType: '%1 expected near \'%2\''
  ...
  function parseRepeatStatement() {
    ...
    var condition = parseExpression();
    if (condition == null)
      raise(token, errors.expectedType, '<expression>', token.value);
  ...
  function parseFunctionDeclaration(name, isLocal) {
    ...
    if (!consume(')'))
      raise(token, errors.expectedType, '<name> or \'...\'', token.value);
    var body = parseBlock();
    expect('end');
  ...
  function parseFunctionName() {
    if (token.type !== Tokens.Identifier)
      raise(token, errors.expectedType, '<name>', token.value);
    var base = ast.identifier(token.value);
    next();
    while (consume('.')) {
      if (token.type !== Tokens.Identifier)
        return raise(token, errors.expectedType, '<name>', token.value);
