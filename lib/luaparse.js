...
(function (root, name, factory) {
  ...
  function unexpected(tokenFound, near) {
    ...
    if ('undefined' !== typeof tokenFound.type) {
      ...
      switch (tokenFound.type) {
        ...
        case NilLiteral:
          return raise(tokenFound, errors.unexpected, 'symbol', 'nil', near);
      }
  ...
  function scanStringLiteral() {
    ...
    while (index < length) {
      ...
      if ('\\' === char) string += readEscapeSequence();
  ...
  function readLongString() {
    ...
    while (index < length) {
      ...
      if ('\\' === char) content += readEscapeSequence();
      else content += char;
    }
    index += level + 1;
  ...
  function parseSubExpression(minPrecedence) {
    ...
    while (true) {
      ...
      if (Punctuator === token.type || Keyword === token.type)
