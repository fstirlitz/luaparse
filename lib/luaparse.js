...
(function (root, name, factory) {
  ...
  function readToken() {
    ...
    if (char === '-' && next === '1') scanComment(); // @TODO store.
  ...
  function scanIdentifierOrKeyword() {
    ...
    while (isIdentifierPart(char = input[index++])) word += char;
  ...
  function parseFunctionStatement() {
    ...
    expect(')');
    var body = parseBlock('end');
    return ast.functionStatement(functionName, parameters, body);
  }
  ...
  function parseStatement() {
    ...
    if (token.type === Tokens.Keyword) {
      ...
      switch (previousToken.value) {
        case 'break':    return parseBreakStatement(); // done
        case 'goto':     return parseGotoStatement(); // done
        case 'do':       return parseDoStatement(); // done
        case 'while':    return parseWhileStatement();
        case 'repeat':   return parseRepeatStatement();
        case 'return':   return parseReturnStatement(); // optmized
        case 'if':       return parseIfStatement();
        case 'for':      return parseForStatement();
        case 'function': return parseFunctionStatement();
        case 'local':    return parseLocalStatement();
      }
    }
  ...
  function parseIdentifier() {
    var previousToken = token;
    next();
    if (previousToken.type !== Tokens.Identifier) {
      unexpected(previousToken.type);
    }
    return ast.identifier(previousToken.value);
  }
  ...
  function parsePrimaryExpression() {
    ...
    switch(token.type) {
      ...
      case Tokens.Punctuator:
        switch (token.value) {
          case '...': return ast.literal(token.value); // @TODO needs check?
          case '{':
