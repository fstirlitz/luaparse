...
(function (root, name, factory) {
  ...
  function unexpected(msg) {
    if (typeof msg === 'undefined') msg = '<eof>';
  ...
  function scanComment() {
    ...
    if (char === '[') {
      ...
      while (index < length) {
        if (isLineTerminator(char = input[index++])) {
          if (options.location) {
            line++;
            lineStart = index;
          }
  ...
  function parseRepeatStatement() {
    ...
    expect('until');
    var condition = parseExpression();
    if (condition == null) unexpected(token.value + ' instead of until expression');
  ...
  function parseReturnStatement() {
    ...
    if (token.value !== 'end') {
      ...
      while (consume(',')) {
        ...
        if (expression == null) unexpected(token.value + ' instead of return explist');
  ...
  function parseFunctionDeclaration(name, isLocal) {
    ...
    if (consume('...')) isVararg = true;
      ...
      do {
        // Vararg must come last in the parameter list.
        if (consume('...')) {
          isVararg = true;
          break;
        }
    ...
    if (expression) {
      if (~',='.indexOf(token.value)) {
        ...
        do {
          ...
          if (exp == null) unexpected('empty expression');
          init.push(exp);
        } while (consume(','));
  ...
  function parseIdentifier() {
    ...
    if (token.type !== Tokens.Identifier) unexpected(token.value);
    next();
