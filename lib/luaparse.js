...
(function (root, name, factory) {
  ...
  function unexpected(tokenFound, near) {
    ...
    if (typeof tokenFound.type !== 'undefined') {
      return raise(tokenFound, errors.unexpected, tokenFound.value, near);
    }
  ...
  function readLongString() {
    ...
    if (input[index + level] !== '[')
      return raise({}, errors.expected, '[', input[index + level]);
  ...
  function parseChunk() {
    ...
    next();
    var body = parseBlock();
    if (token.type !== Tokens.EOF) unexpected(token, '<eof>');
