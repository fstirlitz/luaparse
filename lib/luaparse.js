...
(function (root, name, factory) {
  ...
  function scanHexLiteral() {
    var number = input[index++] + input[index++];

    // A minimum of one hex digit is required.
    if (!isHexDigit(input[index])) raise('malformed number near ' + number);
    while (isHexDigit(input[index])) number += input[index++];
    // Fraction part i optional.
    if (input[index] === '.') {
      number += input[index++];
      // A fraction part will default to 0 in lua.
      while (isHexDigit(input[index])) number += input[index++];
    }
  ...
  function parsePrimaryExpression() {
    ...
    switch (token.type) {
      ...
      case Tokens.HexLiteral:
      case Tokens.DecLiteral:
