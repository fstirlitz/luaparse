...
(function (root, name, factory) {
  ...
  function unexpected(tokenFound, near) {
    ...
    if ('undefined' !== typeof tokenFound.type) {
      var type;
  ...
  function readToken() {
    ...
    // Create the initial token struct containing the start location.
    createToken();

    if (index >= length) return finishToken(EOF, '<eof>');
    ...
    switch (char) {
      ...
      case 91: // [
        ...
        if (91 === next || 61 === next) return scanStringLiteral(true);
  ...
  // Create a new token containing the start location, to be use with the
  // current `readToken()` call.
  ...
  function createToken() {
    ...
    currentToken = {
      ...
      , line: line
      , lineStart: lineStart
      , range: [tokenStart, undefined]
    };
  }

  // Wrap up the the current token with the remaining data and return it
  // ready for the parser.

  function finishToken(type, value) {
    currentToken.type = type;
    currentToken.value = value;
    currentToken.range[1] = index;
    return currentToken;
  }
  ...
  function scanIdentifierOrKeyword() {
    ...
    // Doing a string concatenation amounts for 8% of the entire parse process,
    // slicing the final string is faster.
  ...
  function scanPunctuator(value) {
    index += value.length;
    return finishToken(Punctuator, value);
  }
  ...
  function scanStringLiteral(isLong) {
    var delimiter = input.charAt(index++)
      , string = ''
      , char;
          ...
          switch (input.charAt(index)) {
            ...
            case 'x':
              ...
              if (isHexDigit(input.charCodeAt(index + 1)) &&
                ...
                string += '\\x' + input.charAt(index + 1) + input.charAt(index + 2);
                index += 3;
                break;
              }
              /* falls through */
            default:
              ...
              if (isDecDigit(input.charCodeAt(index))) {
                var byte = '';
                for (var i = 0; i < 3; i++) byte += input.charAt(index++);
                string += '\\' + byte;
                break;
              }
              // Simply remove the \ as it's escaping something.
              string += input.charAt(index++);
          }
        }
    ...
    return finishToken(StringLiteral, string);
  }
  ...
  function scanNumericLiteral() {
    ...
    value = ('0' === char && ~'xX'.indexOf(next || null)) ? readHexLiteral()
      : readDecLiteral();

    return finishToken(NumericLiteral, value);
  }
  ...
  function readHexLiteral() {
    ...
    if (~'pP'.indexOf(input.charAt(index) || null)) {
      ...
      while (isDecDigit(input.charCodeAt(index)))
        binaryExponent += input.charAt(index++);
    ...
    while (index < length) {
      ...
      if (']' === char) {
        ...
        for (var i = 0; i < level; i++) {
          if ('=' !== input.charAt(index + i)) commentTerminator = false;
        }
        if (']' !== input.charAt(index + level)) commentTerminator = false;
      }
  ...
  function isWhiteSpace(char) {
    return 32 === char || 9 === char || 0xB === char || 0xC === char;
  }
  ...
  function binaryPrecedence(operator) {
    ...
    } else if (2 === operator.length) {
      switch (char) {
        case 46: // ..
          if (46 === operator.charCodeAt(1)) return 5;
          break;
        case 60: case 62: case 61: case 126: // <= >= == ~=
          if (61 === operator.charCodeAt(1)) return 3;
          break;
        case 111: // or
          if (114 === operator.charCodeAt(1)) return 1;
          break;
        default: return 0;
      }
